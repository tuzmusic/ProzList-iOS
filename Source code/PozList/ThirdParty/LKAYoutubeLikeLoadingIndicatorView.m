...
- (void)setupLayer {
    ...
    self.viewBGShadow = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
    self.viewBGShadow.backgroundColor = [UIColor clearColor];
    self.viewBGShadow.clipsToBounds = YES;
    self.viewBGShadow.layer.borderWidth = 6;
    self.viewBGShadow.layer.borderColor = [UIColor colorWithRed:242.0/255.0 green:242.0/255.0 blue:242.0/255.0 alpha:1.0].CGColor;
    [self addSubview:self.viewBGShadow];

    [self.layer addSublayer:_circleLayer];
}

- (void)updateLayer {
    CGFloat width = MIN(self.bounds.size.width, self.bounds.size.height) * 0.9;
    ...
    self.viewBGShadow.frame = rect;
    self.viewBGShadow.layer.cornerRadius = rect.size.height / 2.0;
...
- (CAAnimationGroup *)strokeStartAndEndAnimation {
    if (!_strokeStartAndEndAnimation) {
        ...
        strokeStartAnimation.fromValue = @-0.72;
        ...
        strokeStartAndEndAnimation.duration = 1.5;
...
- (void)setLineWidth:(CGFloat)lineWidth {
    ...
    [self stopImplictAnimation:^() {
        ...
        self.viewBGShadow.layer.borderWidth = _lineWidth;
    }];
}
