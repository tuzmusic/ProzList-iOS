...
public final class DropDown: UIView {
	...
	*/
    @objc public dynamic var corner_Radius = DPDConstant.UI.CornerRadius {
		willSet {
    ...
    @objc public dynamic var border_color = DPDConstant.UI.Border_Color {
        willSet {
    ...
    @objc public dynamic var border_width = DPDConstant.UI.Border_Width {
        willSet {
            tableViewContainer.layer.borderWidth = CGFloat(newValue)
        }
        didSet { reloadAllComponents() }
    }
	/**
...
//		tableView.isScrollEnabled = true
	...
	fileprivate func computeLayout() -> (x: CGFloat, y: CGFloat, width: CGFloat, offscreenHeight: CGFloat, visibleHeight: CGFloat, canBeDisplayed: Bool, Direction: Direction) {
			...
			case .any:
				...
				if layout.offscreenHeight > 0 {
					...
					if topLayout.offscreenHeight < layout.offscreenHeight {
						layout = topLayout
                        layout.offscreenHeight = layout.offscreenHeight + 20 //Jigar Patel
						direction = .top
					}
				}
			case .bottom:
        ...
        return (layout.x - 10, layout.y, layout.width, layout.offscreenHeight, (visibleHeight > 60) ? visibleHeight + 10 : visibleHeight , canBeDisplayed, direction)
	}
